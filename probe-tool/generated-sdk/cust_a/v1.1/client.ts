/* Auto-generated by probe-tool Phase 3 */
export class LegacyApiClient_cust_a_v1_1_http_localhost_4013 {
  private readonly capabilityMap = {
    "goal_1_step_preflight": { method: "POST", path: "/api/v1.1/orders/preflight" },
    "goal_1_step_1": { method: "GET", path: "/api/v1.1/orders/template" },
    "goal_1_step_2": { method: "POST", path: "/api/v1.1/orders/{{runId}}-order" },
    "goal_2_step_1": { method: "PUT", path: "/api/{{apiVersion}}/invoices" },
    "goal_3_step_1": { method: "GET", path: "/api/v1.1/expenses/summary" }
  };

  constructor(
    private readonly baseUrl: string,
    private readonly authHeader: Record<string, string>
  ) {}

  async callCapability(
    capabilityId: string,
    request: { path?: string; headers?: Record<string, string>; body?: unknown } = {}
  ): Promise<unknown> {
    const capability = this.capabilityMap[capabilityId as keyof typeof this.capabilityMap];
    if (!capability) {
      throw new Error('Unknown capabilityId: ' + capabilityId);
    }

    request.path = request.path ?? capability.path;
    const response = await fetch(this.baseUrl + request.path, {
      method: capability.method,
      headers: { ...this.authHeader, ...(request.headers ?? {}) },
      body:
        request.body !== undefined && request.body !== null
          ? JSON.stringify(request.body)
          : undefined,
    });

    const text = await response.text();
    let json: unknown = null;
    try {
      json = text ? JSON.parse(text) : null;
    } catch {
      json = text;
    }

    if (!response.ok) {
      throw new Error('SDK call failed (' + response.status + ') for ' + request.path + ': ' + JSON.stringify(json));
    }

    return json;
  }

  async call_goal_1_step_preflight(request: { path?: string; headers?: Record<string, string>; body?: unknown } = {}): Promise<unknown> {
    request.path = request.path ?? "/api/v1.1/orders/preflight";
    const response = await fetch(this.baseUrl + (request.path ?? ""), {
      method: "POST",
      headers: { ...this.authHeader, ...(request.headers ?? {}) },
      body:
        request.body !== undefined && request.body !== null
          ? JSON.stringify(request.body)
          : undefined,
    });

    const text = await response.text();
    let json: unknown = null;
    try {
      json = text ? JSON.parse(text) : null;
    } catch {
      json = text;
    }

    if (!response.ok) {
      throw new Error('SDK call failed (' + response.status + ') for ' + (request.path ?? '') + ': ' + JSON.stringify(json));
    }

    return json;
  }


  async call_goal_1_step_1(request: { path?: string; headers?: Record<string, string>; body?: unknown } = {}): Promise<unknown> {
    request.path = request.path ?? "/api/v1.1/orders/template";
    const response = await fetch(this.baseUrl + (request.path ?? ""), {
      method: "GET",
      headers: { ...this.authHeader, ...(request.headers ?? {}) },
      body:
        request.body !== undefined && request.body !== null
          ? JSON.stringify(request.body)
          : undefined,
    });

    const text = await response.text();
    let json: unknown = null;
    try {
      json = text ? JSON.parse(text) : null;
    } catch {
      json = text;
    }

    if (!response.ok) {
      throw new Error('SDK call failed (' + response.status + ') for ' + (request.path ?? '') + ': ' + JSON.stringify(json));
    }

    return json;
  }


  async call_goal_1_step_2(request: { path?: string; headers?: Record<string, string>; body?: unknown } = {}): Promise<unknown> {
    request.path = request.path ?? "/api/v1.1/orders/{{runId}}-order";
    const response = await fetch(this.baseUrl + (request.path ?? ""), {
      method: "POST",
      headers: { ...this.authHeader, ...(request.headers ?? {}) },
      body:
        request.body !== undefined && request.body !== null
          ? JSON.stringify(request.body)
          : undefined,
    });

    const text = await response.text();
    let json: unknown = null;
    try {
      json = text ? JSON.parse(text) : null;
    } catch {
      json = text;
    }

    if (!response.ok) {
      throw new Error('SDK call failed (' + response.status + ') for ' + (request.path ?? '') + ': ' + JSON.stringify(json));
    }

    return json;
  }


  async call_goal_2_step_1(request: { path?: string; headers?: Record<string, string>; body?: unknown } = {}): Promise<unknown> {
    request.path = request.path ?? "/api/{{apiVersion}}/invoices";
    const response = await fetch(this.baseUrl + (request.path ?? ""), {
      method: "PUT",
      headers: { ...this.authHeader, ...(request.headers ?? {}) },
      body:
        request.body !== undefined && request.body !== null
          ? JSON.stringify(request.body)
          : undefined,
    });

    const text = await response.text();
    let json: unknown = null;
    try {
      json = text ? JSON.parse(text) : null;
    } catch {
      json = text;
    }

    if (!response.ok) {
      throw new Error('SDK call failed (' + response.status + ') for ' + (request.path ?? '') + ': ' + JSON.stringify(json));
    }

    return json;
  }


  async call_goal_3_step_1(request: { path?: string; headers?: Record<string, string>; body?: unknown } = {}): Promise<unknown> {
    request.path = request.path ?? "/api/v1.1/expenses/summary";
    const response = await fetch(this.baseUrl + (request.path ?? ""), {
      method: "GET",
      headers: { ...this.authHeader, ...(request.headers ?? {}) },
      body:
        request.body !== undefined && request.body !== null
          ? JSON.stringify(request.body)
          : undefined,
    });

    const text = await response.text();
    let json: unknown = null;
    try {
      json = text ? JSON.parse(text) : null;
    } catch {
      json = text;
    }

    if (!response.ok) {
      throw new Error('SDK call failed (' + response.status + ') for ' + (request.path ?? '') + ': ' + JSON.stringify(json));
    }

    return json;
  }

}
